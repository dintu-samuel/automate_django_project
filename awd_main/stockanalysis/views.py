...
def stocks(request):
    if request.method =='POST':
        ...
        if form.is_valid():
            stock_id = request.POST.get('stock')   # name = stock
            print('stock_id ==>', stock_id)
            stoc = stock.objects.get(pk=stock_id)
            symbol = stoc.symbol
            exchange = stoc.exchange
            stock_response = scrape_stock_data(symbol, exchange)
            if stock_response:
                try:
                    stock_data = StockData.objects.get(stock=stoc)
                except StockData.DoesNotExist:
                    stock_data = StockData(stock=stoc)
                # update the stockData instance with response data
